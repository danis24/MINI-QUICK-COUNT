<div>
    ...
    @endif
    <form class="max-w mx-auto py-4" wire:submit.prevent="saveCount">
        @if($legislatives->count() > 0)
        @foreach($legislatives as $key => $value)
        <div class="mb-5">
            <label for="email" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">No Urut : {{$value->no_urut}} - {{$value->nama_calon}}</label>
            <input type="number" id="number" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan Jumlah Suara .." wire:model="legislative_data.{{$value->id}}">
        </div>
        @endforeach
        @endif
